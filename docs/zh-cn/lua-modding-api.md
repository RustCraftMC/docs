...
## 3. 生命周期与热重载 {#lifecycle-hot-reload}
...
## 6. 客户端 API：`game.client` {#client-api}
...
## 7. 玩家 API：`game.player` {#player-api}
...
## 10. 世界 API：`game.world` {#world-api}
...
## 11. 网络 API：`game.network` {#network-api}
...
## 12. 协议转换器：`game.protocol` {#protocol-translators}
...
## 13. 第一人称动画 API {#first-person-animation}
...
## 14. HUD 绘制 API：`event.draw` {#hud-drawing}
...
## 16. 资源 API：`game.resources` {#resources-api}
...
## 17. 存储 API：`game.storage` {#storage-api}
